...
def parse_args():
    ...
    parser.add_argument("--dataset-path", type=str, default='all')

    parser.add_argument("--image-base-path", type=str, required=True, help="Base directory containing the 'Images' folder")

    # result_path
    parser.add_argument("--answer-path", type=str, default="./final")
...
def main(args):
    ...
    metrics = evaluate_medical_QA(model=model, dataset=dataset, model_name=args.model_name, dataset_name=os.path.dirname(args.dataset_path).replace('/','_'), time=time, batch_size=args.batch_size, answer_path=answer_path, image_base_path=args.image_base_path)
