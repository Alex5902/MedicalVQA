...
@torch.no_grad()
def test(model, vis_processors, dataset=None, model_type='blip2', prompt_idx=4, save_path='', image_root=''):
    ...
    for data in data_all:
        ...
        if not os.path.exists(img_path):
            print(f"WARNING: Image file not found at constructed path: {img_path}")
...
def parse_args():
    ...
    parser.add_argument("--dataset-path", type=str, required=True, default='/path/to/datset')
    parser.add_argument("--answer-path", type=str, required=True, default="output_res")
    parser.add_argument("--image-root", type=str, required=True, help="Root directory containing the 'Images' folder")
...
def run(args):
    ...
    test(model, vis_processors, dataset=sub_dataset, model_type=model_type, prompt_idx=4, save_path=answer_path, image_root=args.image_root)
